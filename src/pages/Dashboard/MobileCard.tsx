...
interface mobileCardIProps {
  ...
  height: number;
  caseProp?: string;
  userDataFromAllUsers?:any
}

export default function MobileCard({ width, height ,userDataFromAllUsers }: mobileCardIProps) {
...
></i>}
    ...
    <>
     ...
     {  userDataForVisitor ? <>
       ...
       :userDataFromAllUsers? <>
                             ...
                             {userDataFromAllUsers.cards[0].image && userDataFromAllUsers.cards[0].image !== 'add-profile-bigger.jpg' ? (
                                    <div className="h-60 flex justify-center items-center">

                                      <img
                                        src={`https://card.lixir-interiors.com/public/storage/${userDataFromAllUsers.cards[0].image}`}
                                        alt="Uploaded"
                                        className={`h-[100%] w-[100%] ${styles.profileImage} `}
                                      />
                                    </div>
  ...
  {/*  handle svg wave background colors for 3 components userDataForVisitor && userDataForAllUsers && user in setting dashboard */}
    ...
    >
      <path
        fill={ userId&&userDataForVisitor?  userDataForVisitor.cards&&userDataForVisitor.cards[0]?userDataForVisitor.cards[0].background_color :""
                   ...
                   :userDataFromAllUsers?userDataFromAllUsers.cards&&userDataFromAllUsers.cards[0]?userDataFromAllUsers.cards[0].background_color :""
...
{/* handl all social links for 3 components userDataForVisitor && userDataForAllUsers && user in setting dashboard  */}
  ...
  <ul  className="px-3 relative mt-4">
    ...
    {userId ? 
        ...
        <>
               {  userDataForVisitor&&userDataForVisitor.cards[0].card_links.length > 0 ? <> {
    ...
    {
                userDataFromAllUsers.cards[0].card_links.length > 0 ? <>
                ...
                
                {
                          ...
                          userDataFromAllUsers.cards[0].card_links.map((item: any, index: number) => {
                            ...
                            return (
                              ...
                              <li key={index} className="flex items-center mb-3 px-1 py-1 rounded-full " 
                              style={{ border:`1px solid ${userDataFromAllUsers.cards&&userDataFromAllUsers.cards[0]&&userDataFromAllUsers.cards[0].background_color} `}}

                              >
                                <div style={{ backgroundColor:userDataFromAllUsers.cards&&userDataFromAllUsers.cards[0]?userDataFromAllUsers.cards[0].background_color:"" }} className="cyrcleIcon w-9 h-9 rounded-full flex justify-center items-center">
                                  <i style={{ color:userDataFromAllUsers.cards&&userDataFromAllUsers.cards[0]?userDataFromAllUsers.cards[0].icon_color:"" }} className={`${item.logo} text-xl`}></i>
                                </div>
                                {(/^\d+$/.test(item.link)) ? (
                                  <p style={{ color: userDataFromAllUsers.cards&&userDataFromAllUsers.cards[0]?userDataFromAllUsers.cards[0].title_color:""  }} className="ms-2 mb-0 no-underline cursor-pointer " 
                                  onClick={()=> {handleSaveContact(item) } } >{truncatedLink}</p>
                                ) : (
                                  <Link to={item.link} target="_blank" style={{ color: userDataFromAllUsers.cards&&userDataFromAllUsers.cards[0]?userDataFromAllUsers.cards[0].title_color:""  }} className="ms-2 mb-0 no-underline">{truncatedLink}</Link>
                                )}
                              </li>
